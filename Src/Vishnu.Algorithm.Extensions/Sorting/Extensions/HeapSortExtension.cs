...
namespace Vishnu.Extension.Sorting
{
    ...
    public static class HeapSortExtension
    {
        ...
        public static T[] UseHeap<T>(this ISort sort, T[] input, IComparer<T> comparer)
        {
            return sort.Sort<T>(SortingTypes.Heap, input, comparer);
        }
        ...
        /// <param name="comparer">Comparer must returns '1' if the first element is greater than next</param>
        /// <returns>The ascending ordered content</returns>
        public static T[] UseHeap<T>(this ISort sort, ISortingAlgorithmFactory sortingAlgorithmFactory, T[] input, IComparer<T> comparer)
        {
            return sort.Sort<T>(sortingAlgorithmFactory, SortingTypes.Heap, input, comparer);
        }
